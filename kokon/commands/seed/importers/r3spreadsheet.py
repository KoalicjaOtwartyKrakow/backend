...
LUDZI_HEADERS = ['', 'Miejsce zamieszkania gdzie wysłaliśmy ludzi', 'DATA ', 'STATUS', 'imie i nazwisko wolontariusza', 'Imie i Nazwisko', 'Telefon (z kodem kierunkowym)', 'Priorytet', 'TOTAL Ilość osób', 'Dorosly', 'Dzieci', 'Wiek Dzieci np 6m; 5 i 8', 'Zwierzę', 'Uwagi(Czy są kobieta w ciazy, inwalidy (jaka grupa?) Czy sa potrzebny leki, przelewanie krwi dializ etd? Czy są jakies alergie?', 'Finanse', 'Na Ile Czasu', 'Uwagi', 'Apartment Status (do NOT edit here)']

RECEPCJA_HEADERS = ['', 'Notatki', 'Timestamp', "Imię | Ім'я ", 'Nazwisko | Прізвище ', 'Numer kontaktowy | Номер телефону', 'Osoba do kontaktu? | Особа до контакту?', 'Numer dokumentu | Номер паспорту (ID)', 'Ile osób potrzebują zakwaterowania? | Скільки осіб потребують житла? [Mężczyźni | Чоловіки]', 'Ile osób potrzebują zakwaterowania? | Скільки осіб потребують житла? [Kobiety | Жінки]', 'Ile osób potrzebują zakwaterowania? | Скільки осіб потребують житла? [Dzieci | Діти]', 'Wiek dziecka (dzieci) | Вік дитини (дітей)', 'Wiek dorosłej osoby (osób) | Вік дорослої особи (осіб)', 'Zwierzęta domowe | Домашні тварини [Pies | Собака]', 'Zwierzęta domowe | Домашні тварини [Kot | Кіт]', 'Zwierzęta domowe | Домашні тварини [Inne (dopisać niżej) | Інше (дописати внизу)]', 'Na jaki okres potrzebują zakwaterowania? | На який час потрібне житло?', 'Mieszkanie płatne czy za darmo? | Житло платне чи безплатне?', 'Jeśli z dopłatą to ile mogą wydać w miesiącu? | Якщо з доплатою то скільки можуть потратити за місяць?', 'Czy mają własny transport? | Чи є власний транспорт?', 'Miejscowość | Місцевість', 'Od jakiej daty  jest potrzebne zakwaterowanie? | Від якого числа (дати) потрібне житло?', 'Notatki i komentarze dotyczące zwierząt domowych, zakwaterowania, stanu zdrowia, edukacji itd (opcjonalnie) | Нотатки і коментарі відносно домашніх тварин, житла, стану здоров"я, освіти та інше', 'Czy osoba zostaje na noc w naszym punkcie pomocy przy ul. Radziwiłłowska 3? | Чи особа залишається в нашому пункті допомоги на Radziwiłłowska 3?', 'Numer telefonu (wolontariusza akceptującego zgłoszenie) | Номер телефону (волонтера, що приймає заявку)', 'Oświadczenie znajomości reguł przebywania | Підтвердження знайомості правил перебування', 'Czy osoba potrzebuje mieszkania? | Чи особа шукає житло?', '', 'Czy grupa chciałaby pojechać za granicę? | Чи хотіла би група поїхати за кордон?', '']
MIESZKANIA_HEADERS = ['', 'NASZE UWAGI', 'Na jak długo?', 'KIM jest juz zajety? (ID czlowieka) ', 'WOLONTARIUSZ (kto się kontaktował)', 'STATUS', 'Timestamp', 'W jaki sposób chcesz pomóc?', 'Miasto lokalu', 'Adres lokalu', 'Imię i nazwisko:', 'Adres email:', 'Numer telefonu', 'Numer telefonu / adres email', 'Maksymalna ilość ludzi, które możesz przyjąć w swoim lokalu mieszkalnym?', 'Uwagi: Czy posiadasz zwierzęta? Czy akceptujesz gości ze zwierzętami? Godziny w których możemy do Ciebie dzwonić? Masz inne uwagi?', 'Numer telefonu/ adres email', 'Jakie znasz języki?', 'Czy masz doświadczenie w tłumaczeniach?', 'Jeśli tak, czy możesz krótko opisać Twoje doświadczenie?', 'Czy masz doświadczenie w pomocy edukacyjnej?', 'Jeśli tak, czy możesz krótko opisać Twoje doświadczenie?', 'Jeśli masz jakieś pytania, uwagi lub dodatkowe komentarze, na które nie znalazł_ś miejsca w formularzu, napisz je proszę poniżej:', 'Wyrażam zgodę na przetwarzanie moich danych osobowych przez Stowarzyszenie Laboratorium Działań dla Pokoju w celu organizacji pomocy osobom uchodźczym z Ukrainy.', '', '', 'Last modified: Sun Mar 27 2022 13:17:41 GMT+0200 (czas środkowoeuropejski letni)']
...
def import_ludzi():
    ...
    with open("[salam] Ludzi (Ukrainska) - Ludzi.tsv", encoding='utf-8', newline='') as cf:
        ...
        for row in cr:
            ...
            try:
                created_at = datetime.datetime.strptime(column_c, "%d/%m/%Y").isoformat()
            except Exception:
            ...
            try:
                if column_h != '':
                    priority_date = datetime.datetime.strptime(column_h, "%d/%m/%Y").isoformat()
            except Exception:
...
def import_recepcja():
    ...
    with open("[salam] Ludzi (Ukrainska) - UPD Data from Reception.tsv", encoding='utf-8', newline='') as cf:
        ...
        for row in cr:
            ...
            created_at = datetime.datetime.now().isoformat()
            try:
                created_at = datetime.datetime.strptime(column_c, "%d/%m/%Y %H:%M:%S").isoformat()
            except:
            ...
            if adult_male_count_cell != '':
                try:
                    ...
                    validation_notes.append(f'Ile osób potrzebują zakwaterowania? | Скільки осіб потребують житла? [Mężczyźni | Чоловіки]: {column_i}')
            ...
            if adult_female_count_cell != '':
                try:
                    ...
                    validation_notes.append(f'Ile osób potrzebują zakwaterowania? | Скільки осіб потребують житла? [Kobiety | Жінки]: {adult_female_count_cell}')
            ...
            if children_count_cell != '':
                try:
                    ...
                    validation_notes.append(f'Ile osób potrzebują zakwaterowania? | Скільки осіб потребують житла? [Dzieci | Діти]: {children_count_cell}')
            children_ages = [0 for x in range(0, children_count)]
            ...
            validation_notes.append(f'Wiek dziecka (dzieci) | Вік дитини (дітей): {column_l}')

            # Column M Wiek dorosłej osoby (osób) | Вік дорослої особи (осіб) contents SHALL be appended to guests.system_comments
            column_m = row[12]
            validation_notes.append(f'Wiek dorosłej osoby (osób) | Вік дорослої особи (осіб): {column_m}')
            ...
            column_t = row[19]
            validation_notes.append(f'Czy mają własny transport? | Чи є власний транспорт?: {column_t}')
            ...
            try:
                ...
                validation_notes.append(f'Od jakiej daty jest potrzebne zakwaterowanie? | Від якого числа (дати) потрібне житло?: {column_v}')
...
def merge_ludzi_recepcja(ludzi, recepcja):
    ...
    for i, x in enumerate(merged):
        ...
        if r:
            ...
            x['system_comments'].append(f'Could not merge following data:\\n{merge_issues_str}')
            merged[i] = x
    ...
    with open("[salam] Mieszkania (Polska) - Automated Mieszkania.tsv", encoding='utf-8', newline='') as cf:
        ...
        for row in cr:
            ...
            try:
                ...
                unit_validation_notes.append(f'KIM jest juz zajety? (ID czlowieka): {column_d}')
            ...
            try:
                ...
                unit_validation_notes.append(f'Maksymalna ilość ludzi, które możesz przyjąć w swoim lokalu mieszkalnym?: {column_o}')
