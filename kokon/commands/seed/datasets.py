...
def download_data_gov_pl_dataset(temp_path, resource, resources_id):
    ...
    filtered_csv_links = [
        {
            "id": r["id"],
            "url": r["attributes"]["csv_download_url"],
            "size": r["attributes"]["csv_file_size"],
        }
...
def download_first_names(temp_path):
    first_names = download_data_gov_pl_dataset(
        temp_path,
        "1501,lista-imion-wystepujacych-w-rejestrze-pesel",
        [
            male_first_names_id,
...
def download_last_names(temp_path):
    last_names = download_data_gov_pl_dataset(
        temp_path,
        "568,nazwiska-wystepujace-w-rejestrze-pesel",
...
def get_teryt_db_file_name(teryt_path, temp_path, db):
    ...
    with ZipFile(path, "r") as zo:
        ...
        csv_file = f"{stem}.csv"
        if csv_file not in list_of_files:
...
def get_teryt_database(teryt_path, temp_path):
    """unfortunately, Teyrt API needs registering, and
    page uses some ancient ASP.NET post-backs..."""
    ...
    return {"nts": nts, "wmrodz": wmrodz, "simc": simc, "terc": terc, "ulic": ulic}
...
def get_datasets(teryt_path):
    ...
    temp_path.setup()
    datasets = {
        "last_names": download_last_names(temp_path),
        "first_names": download_first_names(temp_path),
